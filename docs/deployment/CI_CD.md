...
| **e2e** | `ubuntu-latest`, `needs: [backend, frontend, mcp, security]` | `npm ci`; `npx playwright install --with-deps chromium`; `docker compose -f docker-compose.yml -f docker-compose.e2e.yml up -d --build`; wait for backend + frontend health; `npm run test:e2e -- --grep-invert "13D Dashboard\|AI Business Insights"` (`E2E_BASE_URL=http://localhost:5173`); upload `playwright-report`; dump stack logs on failure; `down -v` | Runs against the **`e2e`** stack (thin fixed-user seed) — the **25-test security suite** (`auth` / `rbac` / `token-refresh` / `rate-limit-and-features`). The 9-test dashboard/AI suite (`dashboard.spec.ts`) asserts the ~600-order **`demo`** dataset and is excluded here via `--grep-invert`; it is verified against the demo stack instead. |
